...
const AddBus = () => {
  ...
  useEffect(() => {
    const fetchCountries = async () => {
      try {
        ...
        // Push India to the top statically, keeping the rest global securely
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.name || !form.from || !form.to || !form.departureTime || !form.arrivalTime || !form.price || !form.totalSeats) {
      return setMessage("All strictly required fields must be populated ❌");
    }
    if (form.from === form.to) return setMessage("From and To cannot be same ❌");
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <h2 className="text-2xl font-bold mb-4">Add Bus Route 🚌</h2>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <Select options={busOptions} placeholder="Select Bus Type" onChange={(e) => setForm({ ...form, name: e.value })} />
          ...
          <div>
            ...
            <Select 
              ...
              onChange={(e) => setSelectedCountry({ label: e.label, value: e.value })} 
            />
          </div>
          ...
          <div className="bg-gray-50 p-3 rounded border space-y-2 relative">
            <div className="flex justify-between items-center">
              <label className="text-xs font-semibold text-gray-700">Origin Name (Editable)</label>
              <button 
                type="button" 
                onClick={() => setMapSelectionMode(mapSelectionMode === "from" ? null : "from")}
                className={`text-xs px-2 py-1 rounded ${mapSelectionMode === "from" ? "bg-red-500 text-white" : "bg-violet-100 text-violet-700 hover:bg-violet-200"}`}
              >
          ...
          <div className="bg-gray-50 p-3 rounded border space-y-2 relative">
            <div className="flex justify-between items-center">
              <label className="text-xs font-semibold text-gray-700">Destination Name (Editable)</label>
              <button 
                type="button" 
                onClick={() => setMapSelectionMode(mapSelectionMode === "to" ? null : "to")}
                className={`text-xs px-2 py-1 rounded ${mapSelectionMode === "to" ? "bg-red-500 text-white" : "bg-violet-100 text-violet-700 hover:bg-violet-200"}`}
              >
          ...
          <div className="grid grid-cols-2 gap-4">
             <Select options={timeOptions} placeholder="Departure" onChange={(e) => setForm({ ...form, departureTime: e.value })} />
          ...
          <input type="number" placeholder="Total Seats" className="w-full p-3 border rounded" onChange={(e) => setForm({ ...form, totalSeats: e.target.value })} />

          <button disabled={loading} className="w-full bg-violet-600 text-white py-3 rounded hover:bg-violet-700 transition">
            {loading ? "Adding Route..." : "Add Bus Route"}
          </button>
        </form>
        ...
        <div className="bg-white rounded-xl shadow border h-[600px] overflow-hidden relative">
          ...
          {/* Leaflet securely locks dynamically tracking interactions precisely */}
          <MapContainer center={[activeLat, activeLon]} zoom={5} style={{ height: "100%", width: "100%" }}>
            ...
            {coords.fromLat && coords.toLat && (
              <Polyline 
                positions={[[coords.fromLat, coords.fromLon], [coords.toLat, coords.toLon]]} 
                color="blue" 
                weight={4} 
                dashArray="10, 10" 
              />
